...
namespace JosephM.Xrm.Sms.Plugins.Xrm
{
    public class XrmService : IOrganizationService
    {
        ...
        {
            return GetMarketingListMembers(marketingListId, new string[] { Fields.contact_.contactid }).Select(e => e.Id);
        }
        ...
        public IEnumerable<Entity> GetMarketingListMembers(Guid marketingListId, IEnumerable<string> fields)
        {
            ...
            var marketingList = Retrieve(Entities.list, marketingListId, new[] { Fields.list_.type, Fields.list_.query, Fields.list_.createdfromcode });
            var type = marketingList.GetBoolean(Fields.list_.type);
            switch (type)
            {
                case true: //Dynamics
                    {
                        var query = ConvertFetchToQueryExpression(marketingList.GetStringField(Fields.list_.query));
                        query.ColumnSet = CreateColumnSet(fields);
                ...
                case false: //Static
                    {
                        var query = new QueryExpression(Entities.contact)
                        {
                            ColumnSet = CreateColumnSet(fields)
                        };
                        var link = query.AddLink(Relationships.list_.listcontact_association.EntityName, Fields.contact_.contactid, "entityid");
                        link.LinkCriteria.AddCondition(Fields.list_.listid, ConditionOperator.Equal, marketingListId);
                        members.AddRange(RetrieveAll(query));
                        break;
                    }
            }
